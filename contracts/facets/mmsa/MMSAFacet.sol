...
import {
    ExecMode, CallType, ExecType, Execution, ValidationMode, ValidationId, ValidationType
 ...
 */
contract MMSAFacet is IMMSAFacet, IAccount, ModuleManager, ValidationManager, Executor, RegistryAdapter, BarzStorage {
    ...
    address private constant ENTRYPOINT = 0x5FF137D4b0FDCD49DcA30c7CF57E578a026d2789; // v0.6 EntryPoint contract

    // keccak256("Barz7579Message(bytes message)")
    bytes32 private constant MMSA_MSG_TYPEHASH = 0x1efeaafa8dd3a6b211b37d11cde4213d4845ad021eed8dda5ba6003c5c9aa534;
    ...
    function validateUserOp(UserOperation calldata _userOp, bytes32 _userOpHash, uint256 /* missingAccountFunds */ )
        external
        override
    ...
    // |             Validation ID               |
    function decodeNonce(uint256 _nonce)
        internal
        pure
    ...
    function execute(ExecMode _mode, bytes calldata _executionCalldata)
        external
        payable
        override
    ...
    function executeFromExecutor(ExecMode _mode, bytes calldata _executionCalldata)
        external
        payable
        override
        onlyWhenUnlocked
    ...
    function supportsExecutionMode(ExecMode _mode) external pure override returns (bool isSupported) {
        (CallType callType, ExecType execType) = _mode.decodeBasic();

        isSupported = (callType == CALLTYPE_SINGLE || callType == CALLTYPE_BATCH)
            && (execType == EXECTYPE_DEFAULT || execType == EXECTYPE_TRY);
    }

    function supportsModule(uint256 _moduleTypeId) external pure override returns (bool isSupported) {
        isSupported = (_moduleTypeId < 5) ? true : false;
    }

    function getModulesPaginated(uint256 _moduleTypeId, address _start, uint256 _pageSize)
        external
        view
        override
        returns (address[] memory moduleList, address next)
    {
        if (_moduleTypeId == EXECUTOR_MODULE_TYPE) {
            (moduleList, next) = LibMMSAStorage.mmsaStorage().executors.getEntriesPaginated(_start, _pageSize);
        } else if (_moduleTypeId == VALIDATOR_MODULE_TYPE) {
            (moduleList, next) = LibMMSAStorage.mmsaStorage().validators.getEntriesPaginated(_start, _pageSize);
        } else {
    ...
    function uninstallModule(uint256 _moduleTypeId, address _module, bytes calldata _uninitData)
        external
        override
        onlyEntryPointOrSelf
        onlyWhenUnlocked
    ...
    function installValidations(ValidationId[] calldata _vIds, bytes[] calldata _validationData)
        external
        override
        onlyEntryPointOrSelf
        onlyWhenUnlocked
    ...
    {
        isInstalled = _isModuleInstalled(_moduleTypeId, _module, _additionalContext);
    }

    function mmsaFallback(bytes calldata _fallbackData)
        external
        payable
        override
        onlyWhenUnlocked
        returns (bytes memory)
    {
        ...
        FallbackHandler storage fallbackHandler = LibMMSAStorage.mmsaStorage().fallbacks[msgSig];
        ...
        if (calltype == CALLTYPE_STATIC) {
            assembly {
                ...
                if iszero(staticcall(gas(), handler, 0, add(_fallbackData.length, 20), codesize(), 0x00)) {
            ...
            assembly {
                ...
                if iszero(call(gas(), handler, 0, 0, add(_fallbackData.length, 20), codesize(), 0x00)) {
                    returndatacopy(0, 0, returndatasize())
                    revert(0, returndatasize())
                }
    ...
    function mmsaStaticFallback(bytes calldata _fallbackData)
        external
        view
        override
        onlyWhenUnlocked
    ...
    function mmsaIsValidSignature(bytes32 _msgHash, bytes calldata _signature)
        external
        view
        returns (bytes4 magicValue)
    {
        ...
        magicValue =
            IValidator(validator).isValidSignatureWithSender(msg.sender, _getEncodedHash(_msgHash), _signature[20:]);
    }

    function _getEncodedHash(bytes32 _msgHash) internal view returns (bytes32 encodedHash) {
        bytes32 messageHash = keccak256(abi.encode(MMSA_MSG_TYPEHASH, keccak256(abi.encode(_msgHash))));
        encodedHash = keccak256(abi.encodePacked("\x19\x01", _domainSeparator(), messageHash));
    }

    function _domainSeparator() internal view returns (bytes32 domainSeparator) {
        return keccak256(abi.encode(LibVerification.DOMAIN_SEPARATOR_TYPEHASH, block.chainid, address(this)));
    }

    function _decodeAndExecute(ExecMode _mode, bytes calldata _executionCalldata)
        private
        returns (bytes[] memory returnData)
    {
        (CallType callType, ExecType execType) = _mode.decodeBasic();
        ...
        if (callType == CALLTYPE_BATCH) {
            ...
            (address target, uint256 value, bytes calldata callData) = _decodeSingle(_executionCalldata);
