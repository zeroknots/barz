...
library LibUtils {
    ...
    function mergeArrays(bytes4[] memory _array1, bytes4[] memory _array2) internal pure returns (bytes4[] memory) {
    ...
    function removeFacetElement(IDiamondLoupe.Facet[] memory _facets, uint256 _index)
        internal
        pure
        returns (IDiamondLoupe.Facet[] memory)
    {
        ...
        IDiamondLoupe.Facet[] memory newArray = new IDiamondLoupe.Facet[](_facets.length - 1);
    ...
    function removeElement(bytes4[] memory _array, uint256 _index) internal pure returns (bytes4[] memory) {
    ...
    function setValue(bytes4[] memory _keys, address[] memory _values, bytes4 _key, address _value)
        internal
        pure
    ...
    function getValue(bytes4[] memory _keys, address[] memory _values, bytes4 _key) internal pure returns (address) {
        uint256 index = findIndex(_keys, _key);
    ...
    function findIndex(bytes4[] memory _keys, bytes4 _key) internal pure returns (uint256) {
        uint256 keysLength = _keys.length;
        for (uint256 i; i < keysLength;) {
            if (_keys[i] == _key) {
                return i;
