...
library LibDiamond {
    bytes32 constant DIAMOND_STORAGE_POSITION = keccak256("trustwallet.barz.diamond.storage");
    bytes4 constant RESTRICTIONS_FACET_SELECTOR = bytes4(keccak256("verifyRestrictions(address,address,uint256,bytes)"));
    ...
    function restrictionsFacet() internal view returns (address facetAddress_) {
        facetAddress_ = address(bytes20(LibDiamond.diamondStorage().facets[RESTRICTIONS_FACET_SELECTOR]));
    }
}
