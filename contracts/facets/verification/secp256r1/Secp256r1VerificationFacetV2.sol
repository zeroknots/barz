...
import {FCLSecp256} from "./utils/FCLSecp256.sol";
...
contract Secp256r1VerificationFacetV2 is IVerificationFacet, IERC1271 {
        ...
        returns (bool)
    {
        ...
        if (!FCLSecp256.sanity_check([rValue, sValue], q)) {
            return false;
        }

        {
            bytes memory args = abi.encode(sigHash, rValue, sValue, q[0], q[1]);
        ...
        return FCLSecp256.ecdsa_verify(sigHash, [rValue, sValue], q);
    }
}
