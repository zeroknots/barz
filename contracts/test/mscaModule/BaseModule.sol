...
import {IModule, ModuleManifest, ModuleMetadata} from "../../facets/msca/interfaces/IModule.sol";
...
/// happen via the standard execution funtions `execute`, `executeBatch`, and `executeFromModuleExternal`.
/// Note that the modules implement BaseModules cannot be installed when creating an account (aka installed in the
/// account constructor) unless onInstall is overriden without checking codesize of caller (account). Checking
/// codesize of account is to prevent EOA from accidentally calling module and initiate states which will make it
