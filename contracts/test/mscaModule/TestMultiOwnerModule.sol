...
import {SetValue, SIG_VALIDATION_PASSED, SIG_VALIDATION_FAILED} from "../../facets/msca/utils/Constants.sol";
import {BaseModule} from "./BaseModule.sol";
import {IMultiOwnerModule} from "./IMultiOwnerModule.sol";

/// @title Multi Owner Module
/// @author Alchemy
/// @notice This module allows more than one EOA or smart contract to own a modular account.
...
/// such as `TIMESTAMP` or `NUMBER`, and the contract may not be an ERC-1967x
...
contract TestMultiOwnerModule is BaseModule, IMultiOwnerModule, IERC1271 {
    using AssociatedLinkedListSetLib for AssociatedLinkedListSet;
    using ECDSA for bytes32;

    string internal constant _NAME = "Multi Owner Module";
    ...
    function moduleManifest() public pure override returns (ModuleManifest memory) {
        ...
        manifest.userOpValidationFunctions[3] = ManifestAssociatedFunction({
            executionSelector: IModuleManager.installModule.selector,
            associatedFunction: ownerUserOpValidationFunction
        });
        manifest.userOpValidationFunctions[4] = ManifestAssociatedFunction({
            executionSelector: IModuleManager.uninstallModule.selector,
            associatedFunction: ownerUserOpValidationFunction
        });
    ...
    function checkInterface() public pure returns (bytes4) {
        return IStandardExecutor.execute.selector;
    }
    ...
    function ownersOf(address account) public view returns (address[] memory) {
        return LibCast.toAddressArray(_owners.getAll(account));
    }
